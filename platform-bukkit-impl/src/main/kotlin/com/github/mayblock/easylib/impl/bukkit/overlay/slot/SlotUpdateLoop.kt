...
/**
 * 覆盖槽显示更新调度：对每个观察者按 (viewer, slot) 计算显示物品并提交 [SlotDisplayMap]；
 ...
 *
 * **按需运行语义**：仅在有观察者期间运行——由持有者（[com.github.mayblock.easylib.impl.bukkit.overlay.PlayerOverlayImpl]）
    ...
    fun start() {
        ...
        groupsBySlot.forEach { (index, groups) ->
            groups.forEach { (trigger, ordered) ->
                taskIds += scheduler.scheduleSyncTask(trigger) { // 主线程

...
class RealChestMenuClickTest {
    ...
    private fun spec(item: ItemStack, block: (SlotScope<ApiInventoryClickEvent>.() -> Unit)? = null) =
        SlotBuilder(ApiInventoryClickEvent::class.java).apply { item(item); block?.invoke(this) }.build()
    ...
    @Test fun `不可变槽点击被取消`() {
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND))))
        val (_, view) = open(m)
    ...
    @Test fun `声明 onTake 但不放行则拒绝（默认取消）`() {
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND, 2)) { onTake { /* 不设置 isCancelled，保持默认 true */ } }))
        val (_, view) = open(m)
        val e = click(view, 5, InventoryAction.PICKUP_ALL)
        m.handleClick(e)
        assertTrue(e.isCancelled)
    ...
    @Test fun `onTake 显式放行则触发 SlotTakeEvent 且不取消`() {
        var take = 0
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND, 2)) { onTake { isCancelled = false; take++ } }))
        val (_, view) = open(m)
        val e = click(view, 5, InventoryAction.PICKUP_ALL)
        m.handleClick(e)
        assertEquals(1, take)
    ...
    @Test fun `声明 onPlace 但不放行则拒绝（默认取消）`() {
        val m = menu(mapOf(5 to spec(stack(Material.AIR)) { onPlace { } }))
        val (_, view) = open(m)
        view.setCursor(stack(Material.EMERALD, 1))
    ...
    @Test fun `onPlace 显式放行则触发 SlotPlaceEvent 且不取消`() {
        var place = 0
        val m = menu(mapOf(5 to spec(stack(Material.AIR)) { onPlace { isCancelled = false; place++ } }))
        val (p, view) = open(m)
        view.setCursor(stack(Material.EMERALD, 1))
        val e = click(view, 5, InventoryAction.PLACE_ALL)
        m.handleClick(e)
    ...
    @Test fun `信息性 onClick 对已声明槽触发`() {
        var clicks = 0
        val built =
            SlotBuilder(ApiInventoryClickEvent::class.java).apply { item(Material.BARRIER); onClick { clicks++ } }.build()
    ...
    @Test fun `shift 入菜单只向声明了 onPlace 的候选槽分发`() {
        ...
        val m = menu(mapOf(
            0 to spec(stack(Material.STONE, 60)) { onPlace { isCancelled = false } }, // 同类剩 4，放行
            1 to spec(stack(Material.DIAMOND)),                                       // 已声明但无 onPlace：不参与候选
            2 to spec(stack(Material.AIR)) { onPlace { isCancelled = false } },        // 空，放行
        ))
        m.on { on<SlotPlaceEvent> { places += index } }
        val (p, view) = open(m)
        p.inventory.setItem(0, stack(Material.STONE, 40)) // 底部第一格
    ...
    @Test fun `拖拽触及未声明顶部槽则整体取消`() {
        val m = menu(mapOf(0 to spec(stack(Material.AIR)) { onPlace { isCancelled = false } })) // 只声明槽0，槽1 完全未声明
        val (_, view) = open(m)
        view.setCursor(stack(Material.EMERALD, 2))
        val newItems = mapOf(0 to stack(Material.EMERALD, 1), 1 to stack(Material.EMERALD, 1))
        val e = InventoryDragEvent(view, stack(Material.AIR), stack(Material.EMERALD, 2), false, newItems)
        m.handleDrag(e)
        assertTrue(e.isCancelled)
    }
    ...
    @Test fun `拖拽触及已声明但未放行的顶部槽仍整体取消（事件契约默认拒绝）`() {
        val m = menu(mapOf(
            0 to spec(stack(Material.AIR)) { onPlace { isCancelled = false } },
            1 to spec(stack(Material.AIR)) { onPlace { } }, // 声明但不放行
        ))
        val (_, view) = open(m)
        view.setCursor(stack(Material.EMERALD, 2))
        val newItems = mapOf(0 to stack(Material.EMERALD, 1), 1 to stack(Material.EMERALD, 1))
    ...
    @Test fun `拖拽仅触及已放行的顶部槽则放行并逐槽 onPlace`() {
        ...
        val m = menu(
            mapOf(
                0 to spec(stack(Material.AIR)) { onPlace { isCancelled = false; place++ } },
                1 to spec(stack(Material.AIR)) { onPlace { isCancelled = false; place++ } },
            )
        )
        val (_, view) = open(m)
        view.setCursor(stack(Material.EMERALD, 2))
    ...
    @Test fun `FireSwap 均显式放行则触发 take 与 place 且不取消`() {
        ...
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND, 1)) {
    ...
    @Test fun `shift 分发中某槽未放行则跳过、扣除只计实际放入`() {
        val m = menu(mapOf(
            0 to spec(stack(Material.AIR)) { onPlace { /* 不放行 */ } },
            1 to spec(stack(Material.AIR)) { onPlace { isCancelled = false } },
        ))
        val (p, view) = open(m)
        p.inventory.setItem(0, stack(Material.STONE, 100))
    ...
    @Test fun `MenuInteractionListener 按 holder 路由点击到菜单`() {
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND)))) // 无 handler 的声明槽
    ...
    @Test fun `专项① 声明 onTake 不调用放行则始终拒绝`() {
        val m = menu(mapOf(5 to spec(stack(Material.DIAMOND, 1)) { onTake { /* no-op */ } }))
        val (_, view) = open(m)
        val e = click(view, 5, InventoryAction.PICKUP_ALL)
        m.handleClick(e)
        assertTrue(e.isCancelled)
    ...
    @Test fun `专项② onTake 按物品数量条件放行`() {
        val allowMenu = menu(mapOf(5 to spec(stack(Material.DIAMOND, 2)) {
        ...
        val denyMenu = menu(mapOf(5 to spec(stack(Material.DIAMOND, 1)) {
            onTake { isCancelled = item.amount < 2 }
        }))
    ...
    @Test fun `专项④ 外部总线订阅者可放行 cursor-place`() {
        ...
        val m = menu(mapOf(5 to spec(stack(Material.AIR))))
